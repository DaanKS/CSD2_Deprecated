...
int main(int argc,char **argv)
{
  ...
  RingMod sine1(samplerate, 440.0, 1.0);
  RingMod sine2(samplerate, 440.0, 1.0);


  //assign a function to the JackModule::onProces
  jack.onProcess = [&sine1, &sine2](jack_default_audio_sample_t *inBuf,
    ...
    for(unsigned int i = 0; i < nframes; i++) {
      ...
      outBuf[i] = (sine1.getSample() * amplitude) * (((sine2.getSample() + 1.0) / 2) * amplitude);
      sine1.tick();
      sine2.tick();
    }
    return 0;
  ...
  while (running)
  {
    switch (std::cin.get())
    {
      ...
      sine1.getFrequency();
      sine2.setFrequency(frequency / 1.3);
      sine2.getFrequency();
        break;
      }
      case 'b':
